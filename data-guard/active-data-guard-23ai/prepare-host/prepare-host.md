...
9. Open the **Cloud Shell** using the icon next to the region.
  ...
  You can maximize it for a better experience. If it asks to run a tutorial, you can enter `N`.
...
4. Execute the preparation script also on the secondary host. This will:
    * Create the static service registration entry in listener.ora
    * Create the application TNS entries in tnsnames.ora
    ```
    <copy>
...
* The `DGConnectIdentifier`, which should connect to the default service named after the `DB_UNIQUE_NAME` or another service accessible when the instance is mounted.
* The `StaticConnectIdentifier`, which should connect to the service statically registered with the listener (we created it in the previous step).
...
2. With the editor of your choice, add the two connection descriptors to the tnsnames.ora **of both servers.**
    ```
    <copy>
...
4. With the editor of your choice, add the two connection descriptors to the tnsnames.ora **of both servers.**
    ```
    <copy>
    vi $ORACLE_HOME/network/admin/tnsnames.ora
    </copy>
...
5. Verify that both servers contain the four connection descriptors.
    **It is very important that the tnsnames.ora of each server resolves all the four connection descriptors:**
    ```
    <copy>
